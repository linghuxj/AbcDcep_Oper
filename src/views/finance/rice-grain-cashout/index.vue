...
<template>
  <div class="main-content">
    <div v-if="permissions.withdraw_conf" style="margin-bottom: 10px">
      ...
      <el-button
        type="primary"
        @click="riceGrainSetUp()"
        v-if="permissions.store_integral_withdraw_conf"
        >米粒提现设置</el-button
      >
    </div>
    <el-form :inline="true" :model="search">
      <el-form-item label="商户搜索：">
        <el-select
          ...
          style="width: 30%"
        >
          <el-option label="店铺名" value="storeName"></el-option>
          <el-option label="手机号" value="loginloginMobile"></el-option>
        </el-select>
        <el-input
      ...
      <el-form-item label="提现单号：">
        <el-input
          v-model="parameter.withdrawOrderId"
          placeholder="请输入提现单号"
        ></el-input>
      </el-form-item>
      <el-form-item label="提现状态：">
        <el-select v-model="parameter.payStatus" placeholder="请选择">
          <el-option
            v-for="(item, index) in payStatusAll"
            :key="index"
            :label="item.dictName"
      ...
      <el-form-item label="提现类型：">
        <el-select v-model="parameter.walletType" placeholder="请选择">
          <el-option
            v-for="(item, index) in walletTypeArr"
            :key="index"
            :label="item.dictName"
            :value="item.dictValue"
          />
    ...
    <div style="margin-top: 10px">
      <el-tabs v-model="interactive.activeName" @tab-click="handleClick">
        ...
        <el-tab-pane label="审核已通过" name="normal"> </el-tab-pane>
        <el-tab-pane label="审核不通过" name="failed"> </el-tab-pane>
      </el-tabs>
      <el-table
      ...
      >
        ...
        <el-table-column label="商户信息">
          <template slot-scope="scope">
            <div>{{ numFilter(scope.row.store) }}</div>
            <div>{{ numloginMobile(scope.row.store) }}</div>
          </template>
        </el-table-column>
        ...
        <el-table-column
          v-if="interactive.activeName === 'normal'"
          key="status"
          prop="payStatusName"
          label="提现状态"
        />
        ...
        /> -->
        <el-table-column
          v-if="interactive.activeName != 'audit'"
          label="审核人信息"
        >
          <template slot-scope="scope">
            <div>{{ nameFilter(scope.row.auditUser) }}</div>
        ...
        />
        <el-table-column
          v-if="interactive.activeName === 'failed'"
          prop="remark"
          label="原因"
        />
...
import {
  getStoreWithdrawPage,
  upStoreWithdrawPayStatus,
  upStoreWithdrawUpdateStatus,
  upStoreWithdrawTypes
} from '@/api/finance/store-withdrawal'
...
export default {
  ...
  methods: {
    ...
    // 米粒提现设置
    riceGrainSetUp() {
      this.$router.push('/finance/rice-grain-set')
    },
    // 查询店铺类型
    getWalletType() {
      upStoreWithdrawTypes().then(res => {
    ...
    getWithdrawPayStatus() {
      upStoreWithdrawPayStatus().then(res => {
    ...
    getList() {
      ...
      getStoreWithdrawPage(parameter)
        .then(resp => {
    ...
    submitToExamine() {
      ...
      upStoreWithdrawUpdateStatus(this.temp).then(() => {
        this.$message({
          type: 'success',
          message: '操作成功！'
        })
