<template>
  ...
  <div class="main-content ayment">
    ...
    >
      ...
      <template v-if="dialogTable.length > 0 && rowData.status == 'wait_pay'">
        <p>结算金额（元）：{{ dialogTable.amountSum }}</p>
        <p>手续费（元）：{{ dialogTable.chargeAmountSum }}</p>
        <el-form
          :inline="true"
          :model="payForm"
          ref="payForm"
      ...
      <div v-else style="text-align: right">
        <p>结算金额（元）：{{ dialogTable.amountSum }}</p>
        <p>手续费（元）：{{ dialogTable.chargeAmountSum }}</p>
        <p>结算渠道：{{ rowData.payChannelName }}</p>
      </div>
      <span slot="footer" class="dialog-footer">
        <el-button @click="dialogVisible = false">{{
          rowData.status == 'wait_pay' ? '取 消' : '关闭'
        }}</el-button>
        <el-button
          v-if="rowData.status == 'wait_pay'"
          type="primary"
          @click="havePay"
          >确认付款</el-button
        >
...
import { getpayTypes } from '@/api/finance/set'
export default {
  ...
  methods: {
    getList(type) {
      ...
      const [payStartDate, payEndDate] = this.datetime
      // 拼接请求参数
      const listQuery = {
        payStartDate,
        payEndDate,
        ...
        status:
          this.parameter.status == 'all' ? undefined : this.parameter.status,
    ...
    showDetail(rowData) {
      ...
      getBookItemList(rowData.bookId).then(res => {
        this.dialogTable = res.data.data
        this.dialogTable.amountSum = 0
        this.dialogTable.chargeAmountSum = 0
        this.dialogTable.forEach(item => {
          this.dialogTable.amountSum += item.payAmount * 1000 || 0
          this.dialogTable.chargeAmountSum += item.chargeAmount * 1000 || 0
        })
        this.dialogTable.amountSum = (
          this.dialogTable.amountSum / 1000
        ).toFixed(2)
        this.dialogTable.chargeAmountSum = (
          this.dialogTable.chargeAmountSum / 1000
        ).toFixed(2)
        this.tableLoading = false
      })
    ...
    // 导出
    potentialExport() {
      this.$confirm('确认将台账付款导出？', '导出台账付款', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      })
        .then(() => {
          getDownloadItem(this.rowData.bookId).then(res => {
            this.downloadExcel(res, (this.rowData.name || '') + '台账付款.xls')
            console.log(res)
          })
        })
        .catch(() => {
