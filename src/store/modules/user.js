...
import { getMenu, getTopMenu } from '@/api/user/menu'
import { getMine } from '@/api/user/user'
    ...
    LoginByUsername({ commit }, userInfo) {
      ...
      return new Promise((resolve, reject) => {
        loginByUsername(user.username, user.password, user.code, user.randomStr).then(response => {
          ...
          commit('SET_USER_ID', data.details_user.username)
          commit('SET_PERMISSIONS', data.details_user.authorities || [])
          commit('CLEAR_LOCK')
          resolve()
        }).catch(error => {
          reject(error)
